...
if st.session_state["page"] == "home_page":
    ...
    [data-testid="stAppViewContainer"] {
        background-image: url("https://i.postimg.cc/HWftM8w6/back.jpg");
    ...
    .stMarkdown p, .stMarkdown li, .stText, .stSpinner {
        color: black !important;
    }
    .tool-card {
    ...
    st.subheader("Smart tools for data-driven content creation")

    # Brief description
    st.markdown("""
    Create high-performing banners and notifications using AI-powered insights from historical data.
    """)
    ...
    st.markdown("### 🎨 Banner Generation Tool")
    st.markdown("""
    - Generate data-driven banners using AI
    - Add custom text overlay to designs
    """)
    
    if st.button("Launch Banner Generator", key="banner_tool", use_container_width=True):
        st.success("🎯 Launching Banner Generation Tool...")
        with st.spinner("⏳ Loading data... Please wait."):
            time.sleep(2) 
            st.session_state["page"] = "banner"
            st.rerun()
        # Add your banner tool navigation logic here

    st.divider()

    st.markdown("### 📢 Notification Content Generator")
    st.markdown("""
    - Generate content based on past high CTR data
    - Create notifications with proven click rates
    """)
    
    if st.button("Launch Notification Generator", key="notification_tool", use_container_width=True):
        st.success("🎯 Launching Notification Content Generator...")
        with st.spinner("⏳ Loading data... Please wait."):
            time.sleep(2) 
            st.session_state["page"] = "content"
            st.rerun()
        # Add your notification tool navigation logic here
...
if st.session_state["page"] == "content":
    ...
    top_template_count = st.number_input(
        "Enter how many top templates you want from the initial data:", value=1 ,min_value=1
    )

    if option:
       
        df = main_df[main_df['Bussiness'] == option]

        df["Merged"] = df["Message_title"] + " " + df["Message_body"]
        df.drop(["Event_Captured_DT", "Identity"], axis=1, inplace=True)


        pysql = df.groupby("Merged").agg(Notification_Clicked=('Notification_Clicked', 'sum'),  Push_Imp=('PUSH_IMPRESSION', 'sum')).reset_index()
        pysql["CTR"] = pysql["Notification_Clicked"] / pysql["Push_Imp"]

       
        nad = pysql.sort_values(by='CTR', ascending=False)
    
        tb_df = nad.head(top_template_count)
        with st.expander("Expand to See the DataFrame"):
          st.dataframe(tb_df)
    ...
    if st.button("Generate Templates"):
            ...
            messages_2 = [
                ("system","""You are a creative AI assistant specialized in writing engaging notification templates. and please give me in a json file way and i want {"template 1":{"message title":{},"message body":{}},"template 2:{}}"""),
                ("human", user_message)
            ]
            ...
            main_json_dup = str(ai_response.content)
            start_idx = main_json_dup.find("{")
            end_idx = main_json_dup.rfind("}")
            main_cleaned = main_json_dup[start_idx:end_idx+1]
           
            cleaned_json = json.loads(main_cleaned.strip())
            main_len = len(cleaned_json)
            for i in range(main_len):
             with st.container(border=True):
                st.write(cleaned_json[f"template {i+1}"]["message title"])
                st.write(cleaned_json[f"template {i+1}"]["message body"])

    home_btn = st.button("Back to Home", icon="⬅️")
    if home_btn:
        with st.spinner("⏳ Loading data... Please wait."):
            time.sleep(2)
            st.session_state["page"] = "home_page"
...
# ----------------------------------------------------------------
       ...
       if st.session_state.img_btn_clicked and not st.session_state.img_already_generated:
        ...
        if API_KEY:
            human_message = (
                ...
                "The image should look grounded, contextually relevant, and visually aligned with professional Indian financial service advertising. and if animated please give me in cartoonish way"
            )
            ...
            • Avoid clutter, overuse of background fillers, or elements not described in the prompt. and don't include plants on the coin for fixed depoist.
            ...
            def image_generation_using_flux_1_pro(sharedurls, promptt):
                try:
                    ...
                    if resp.status_code == 200:
                        # Check if response looks like an image (binary content) or an error message
                        content_type = resp.headers.get("Content-Type", "")
                        
                        if "image" in content_type.lower():
                            sharedurls.append(resp.content)
                        else:
                            # Check if the text contains an error message like "502"
                            text_content = resp.text.lower()
                            if "502" in text_content or "error" in text_content:
                                st.error("Image generation failed due to server error (502 upstream or similar).")
                            else:
                                st.error("Unexpected response format. No image returned.")
                    else:
    ...
    with d_lef:
            ...
            with st.expander("Want to add the exact text on the banner?"):
               oth_text_overlay =  st.text_input("Enter the desired text")
               generate_banner_with_exact_text = st.button("Generate the text overlay banner")
            generate_text_overlay_content_btn = st.button("Generate AI-Based Content")
    ...
    with d_lef:
        ...
        TEXT_OVERLAY_API_KEY = os.getenv("GOOGLE_API_KEY")
...
# Configure the API key
    ...
    with d_rig:
        ...
        if selected and oth_text_overlay and generate_banner_with_exact_text:
            if length_total_rec >= 1:
              for i, image_ in enumerate(selected):
                try:
                    with st.spinner("💥 Adding Text Overlay To Images.."):
                        ...
                        prompt = f"""Add "{oth_text_overlay}" in Rubik bold font, white/black high contrast color. Place LEFT SIDE in free space or if there is no free space in the left side place in the available free space area and don't change the original image at all, NO OVERLAP with objects. Professional banner style, large readable text, proper spacing. CRITICAL: Ensure typography is crystal clear, sharp, and not blurry - use high resolution text rendering with anti-aliasing for maximum readability and professional appearance. and upscale the image background but don't change and upscale the persons eyes and faces if had """
